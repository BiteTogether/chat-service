...
public class MessageService {
  ...
  public Mono<ApiResponseDTO<ChatMessageDTO>> processIncoming(ChatInboundMessage inbound, Long senderId) {
    return validateMember(inbound.getConversationId(), senderId)
        ...
        .flatMap(savedMessage -> updateConversationLastMessageTime(savedMessage)
            .then(toDecryptedDTO(savedMessage)))
  ...
  /**
   * Get messages by conversation ID with cursor-based pagination.
   * Messages are returned in descending order (newest first).
   * Automatically marks fetched messages as read for the current user.
   *
  ...
  public Mono<ApiResponseDTO<MessagePageResponse>> getMessagesByConversationId(
    ...
    return ReactiveUserContextUtils.getUserIdOrError("User ID not found in context")
        .flatMap(userId -> validateMember(conversationId, userId)
            .then(fetchMessages(conversationId, cursor, limit))
            .flatMap(response -> markAsRead(conversationId, userId, response.getData())
                .thenReturn(response)));
  }

  /**
   * Mark messages as read up to the highest sequence number in the fetched messages.
   * This is called automatically when fetching messages.
   */
  private Mono<Void> markAsRead(String conversationId, Long userId, MessagePageResponse response) {
    ...
    Long highestSeq = response.getMessages().stream()
        .mapToLong(ChatMessageDTO::getSeq)
        .max()
        .orElse(0L);
  ...
  /**
   * Update the last read sequence for a user in a conversation.
   * Only updates if the new sequence is higher than the current one.
   */
  private Mono<Void> updateLastReadSequence(String conversationId, Long userId, Long lastReadSequence) {
  ...
  private ApiResponseDTO<MessagePageResponse> buildPageResponse(
    ...
    CursorPageResult<ChatMessageDTO, Long> pageResult = PaginationUtils.buildCursorPageResult(
        messages,
        requestedLimit,
        ChatMessageDTO::getSeq);
  ...
  public Mono<ApiResponseDTO<ChatMessageDTO>> getMessageById(String messageId) {
    return ReactiveUserContextUtils.getUserIdOrError("User ID not found in context")
        .flatMap(userId -> messageRepository.findById(messageId)
            .switchIfEmpty(Mono.error(new AppException(ErrorCode.MESSAGE_NOT_FOUND)))
            .flatMap(message -> validateMember(message.getConversationId(), userId)
   ...
   * Publishes a MessageUpdatedEvent for real-time updates.
   *
   * @param messageId the message ID
   * @param request   the update request containing new content
   * @return the updated message
   */
  public Mono<ApiResponseDTO<ChatMessageDTO>> updateMessage(String messageId, UpdateMessageRequest request) {
    return ReactiveUserContextUtils.getUserIdOrError("User ID not found in context")
        .flatMap(userId -> messageRepository.findById(messageId)
            .switchIfEmpty(Mono.error(new AppException(ErrorCode.MESSAGE_NOT_FOUND)))
            .flatMap(message -> validateMessageOwner(message, userId)
                .then(validateMember(message.getConversationId(), userId))
                .then(updateMessageContent(message, request.getContent()))));
  }

  private Mono<ApiResponseDTO<ChatMessageDTO>> updateMessageContent(Message message, String newContent) {
    return cryptoService.encrypt(newContent)
        .flatMap(encrypted -> {
        ...
        .flatMap(this::toDecryptedDTO)
        .doOnNext(dto -> eventPublisher.publishMessageUpdated(
   ...
   * Publishes a MessageDeletedEvent for real-time updates.
   *
   * @param messageId the message ID
   * @return void
   */
  ...
  private Mono<ApiResponseDTO<Void>> deleteMessageAndPublish(Message message) {
    ...
    return messageRepository.delete(message)
        .doOnSuccess(unused -> eventPublisher.publishMessageDeleted(
