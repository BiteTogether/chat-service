...
public class ChatWebSocketHandler implements WebSocketHandler {
        ...
        .flatMap(userContext -> {
          ...
          return subscribeToAllConversations(session, userId)
              .then(session
                  ...
                  .flatMap(payload -> handlePayload(payload, session, userId)
                      .onErrorResume(e -> {
                  ...
                  .doFinally(s -> {
                    log.info("WebSocket session closed: sessionId={}, userId={}", session.getId(), userId);
  ...
  private Mono<Void> subscribeToAllConversations(WebSocketSession session, Long userId) {
    return participantRepository.findByUserId(userId)
        .doOnNext(participant -> {
          log.info("Auto-subscribing session {} to conversation {}", session.getId(), participant.getConversationId());
  ...
  private UserContext extractUserContext(WebSocketSession session) {
    return webSocketAuthService.extractUserContext(
    ...
    return Mono.fromCallable(() -> mapper.readValue(payload, ChatInboundMessage.class))
        .flatMap(msg -> {
          return switch (msg.getAction()) {
            case SUBSCRIBE -> {
              log.info("Session {} subscribed to room {}", session.getId(), msg.getConversationId());
              registry.join(msg.getConversationId(), session);
              yield Mono.empty();
            }
            case UNSUBSCRIBE -> {
              log.info("Session {} unsubscribed from room {}", session.getId(), msg.getConversationId());
              registry.leave(msg.getConversationId(), session);
              yield Mono.empty();
            }
            case SEND -> messageService
