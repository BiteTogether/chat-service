...
public final class PaginationUtils {
  ...
  public static <T, C> CursorPageResult<T, C> buildCursorPageResult(
      List<T> items, Integer requestedLimit, Function<T, C> cursorExtractor) {
    ...
    List<T> resultItems = hasMore ? items.subList(0, pageSize) : items;

    C nextCursor = resultItems.isEmpty() ? null : cursorExtractor.apply(resultItems.getLast());
